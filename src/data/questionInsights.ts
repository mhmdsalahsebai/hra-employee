/* ───────────────────────────────────────────────────────────────────────────
   Per-question insights shown *during* the assessment, right after the person
   answers. Frontend mirror of data/hraQuestionInsights.ts (the backend-shaped
   source of truth) — regenerate rather than editing entries here by hand.

   Every entry is grounded in a real, named, respectable source — WHO, Gallup,
   The Lancet, BMJ, JAMA, PNAS, Science, named university studies — with the
   actual figure that research reports. The stat is the hook the card animates;
   the text is the takeaway; the source line anchors it in evidence.

   Keyed by the question `slug` (the enum-member name used across
   hra.generated.ts). Questions without an entry keep the quiz's normal
   auto-advance.
   ─────────────────────────────────────────────────────────────────────────── */

export type QuestionInsightKind = "fact" | "encouragement";

export interface InsightStat {
  /** The figure the card counts up to. Integers only — it animates. */
  value: number;
...
/** Human label for each kind, used by the UI chip. */
...
const questionInsights: Record<string, QuestionInsight> = {
  ...
  IndifferenceColleaguesWellbeing: {
    kind: "encouragement",
